...
@section('blogpage-content')
    ...
    <div class="table-search-box">
        <div class="table-search-result-title">
            @if (!empty($search))
                {{ __('elfcms::default.search_result_for') }} "{{ $search }}" <a href="{{ route('admin.blog.categories') }}" title="{{ __('elfcms::default.reset_search') }}">&#215;</a>
            @endif
        </div>
            ...
            @foreach ($categories as $category)
                <tr data-id="{{ $category->id }}" class="level-{{ $category->level }}@empty ($category->active) inactive @endempty">
                    ...
                    <td class="button-column non-text-buttons">
                        ...
                        <a href="{{ route('admin.blog.categories.edit',$category->id) }}" class="default-btn edit-button" title="{{ __('elfcms::default.edit') }}"></a>
                        ...
                        <div class="contextmenu-content-box">
                            <a href="{{ route('admin.blog.posts',UrlParams::addArr(['category'=>$category->id])) }}" class="contextmenu-post">
                            ...
                            <a href="{{ route('admin.blog.categories.edit',$category->id) }}" class="contextmenu-post">{{ __('elfcms::default.edit') }}</a>
                            <form action="{{ route('admin.blog.categories.update',$category->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <input type="hidden" name="id" id="id" value="{{ $category->id }}">
