<div class="blog-nav-cat blog-nav-line" data-id="{{ $cat->id }}" data-line="category">
    <div class="blog-line-position" data-line="category" draggable="true">{{ $cat->position }}</div>
    ...
    <form action="{{ route('admin.blog.blogs.destroy',$cat) }}" method="POST" data-submit="check" data-header="{{ __('blog::default.deleting_of_blog',['blog'=>$cat->name]) }}" data-message="{{ __('blog::default.are_you_sure_to_deleting_blog') }}">
        @csrf
        @method('DELETE')
