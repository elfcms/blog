<div class="blog-nav-el blog-nav-line" data-line="post" data-id="{{ $post->id }}">
    <div class="blog-line-position" data-line="post" draggable="true">{{ $post->position }}</div>
    ...
    <form action="{{ route('admin.blog.blogs.destroy',$post) }}" method="POST" data-submit="check" data-header="{{ __('blog::default.deleting_of_blog',['blog'=>$post->name]) }}" data-message="{{ __('blog::default.are_you_sure_to_deleting_blog') }}">
        @csrf
        @method('DELETE')
