...
@section('blogpage-content')
    ...
    <div class="item-form">
        <h3>{{ __('basic::elf.create_comment') }}</h3>
                ...
                <div class="input-box colored">
                    ...
                    <div class="input-wrapper">
                        ...
                        @foreach ($posts as $post)
                            <option value="{{ $post->id }}" @if ($post->active != 1) class="inactive" @endif @if ($post->id == $comment->post_id) selected @endif>{{ $post->name }}@if ($post->active != 1) [{{ __('basic::elf.inactive') }}] @endif</option>
                        @endforeach
                        </select>
                    </div>
                </div>
                <div class="input-box colored">
                    ...
                    <div class="input-wrapper">
                        ...
                        @foreach ($comments as $item)
                            <option value="{{ $item->id }}" @if ($item->active != 1) class="inactive" @endif @if ($item->id == $comment->parent_id) selected @endif>#{{ $item->id }} ({{ Str::limit($item->text, 15) }})@if ($item->active != 1) [{{ __('basic::elf.inactive') }}] @endif</option>
                        @endforeach
                        </select>
                    </div>
                </div>
